...
def find_light(img_binary, img, mode):
    ...
    lights_filtered = [
        ...
        if cv2.contourArea(contour) > 5 and -30 <= adjust(cv2.minAreaRect(contour))[2] <= 30  # 过滤小的轮廓和大于-30°-30°的旋转矩形
    ]
    # 进一步过滤重叠的光源
    lights_filtered = [
        light for light in lights_filtered 
        if not any(is_coincide(cv2.boxPoints(light).astype(int), cv2.boxPoints(other_light).astype(int)) for other_light in lights_filtered if light != other_light)  # 检查重叠
    ]
